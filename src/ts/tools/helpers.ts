            ...
            $.each(composes, (idx: number, item: GmailDomCompose) => {
                ...
                sendBtn.on("click", (e) => {
                    ...
                    if (bccSalesforce) {
                        ...
                        const toList: string[] = [];
                        $.map($toList.siblings().children("span[email]"), (element) => {
                            toList.push($(element).attr("email"));
                        });

                        const $ccList: JQuery = item.cc() as any;
                        const ccList: string[] = [];
                        $.map($ccList.siblings().children("span[email]"), (element) => {
                            ccList.push($(element).attr("email"));
                        });

                        const $bccList: JQuery = item.bcc() as any;
                        const bccList: string[] = [];
                        $.map($bccList.siblings().children("span[email]"), (element) => {
                            bccList.push($(element).attr("email"));
                        });
